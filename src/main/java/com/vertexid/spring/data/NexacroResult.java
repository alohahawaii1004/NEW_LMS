...
public class NexacroResult {
     ...
     * @param beans
     */
    private void addDataSet(String dataSetName, List<?> beans) {
        checkName(dataSetName);
        checkBean(beans);
        dataSetMaps.put(dataSetName, beans);
    }
    
    public void addDataSet(String dataSetName, List<?> beans, Class<?> clz) {
		if(beans == null || beans.size() == 0) {
			addDataSet(dataSetName, new BeanMetaData(clz));
		}else{
			addDataSet(dataSetName, beans);
		}
    }
     ...
     * @param beans
     */
    private void addDataSet(String dataSetName, Object beans) {
        checkName(dataSetName);
        checkBean(beans);
        dataSetMaps.put(dataSetName, beans);
    }
    
    public void addDataSet(String dataSetName, Object beans, Class<?> clz) {
        checkName(dataSetName);
        checkBean(beans);
        if(beans == null ) {
        	addDataSet(dataSetName, new BeanMetaData(clz));
        	//TODO 테스트
//        	addDataSet(dataSetName, new BeanMetaData(clz).getMetaData());
        }else{
        	addDataSet(dataSetName, beans);
        }
    }
