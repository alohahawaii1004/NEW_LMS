...
(function($){
    ...
    function toHtml(target, rows, footer, caption, fields){
        ...
        for(var i=0; i<fields.length; i++){
            ...
            //-- 강세원 커스터마이징 hidden 필드는 다운로드 하지 않는다.
            var colDisplayFlag = (typeof col.toExcel !== "undefined") ? col.toExcel : true;

            if(colDisplayFlag && !col.hidden) {
                data.push('<th style="' + tdStyle + '">' + filterXSS(col.title) + '</th>');
            }
        ...
        $.map(rows, function(row){
            ...
            for(var i=0; i<fields.length; i++){
                ...
                var tdStyle = tdStyle0 + ";mso-number-format:'\@'";    // 숫자형을 문자형으로 지정 ex) 01 -> 1 '0'이 사라지지 않게 하기 위함
                ...
                //-- 강세원 커스터마이징 hidden 필드는 다운로드 하지 않는다.
                var colDisplayFlag = (typeof col.toExcel !== "undefined") ? col.toExcel : true;
                var excelFormatterFlag = (typeof col.excelFormatter !== "undefined") ? col.excelFormatter : true;

                if(colDisplayFlag && !col.hidden) {
                    if(col.formatter && excelFormatterFlag){
                        data.push(
                            '<td style="'+tdStyle+'">'+col.formatter(value, row, i)+'</td>'
                        );
                    }else{
                        data.push(
                            '<td style="'+tdStyle+'">'+filterXSS(value)+'</td>'
                        );
                    }
                }
            }
    ...
    function toXLSX(target, param){
        ...
        $("body").append($("<div style='display:none;' id='testExport'>").html(table));

        var wb = XLSX.utils.table_to_book(document.getElementById("testExport").getElementsByTagName("table")[0], {raw:true});
