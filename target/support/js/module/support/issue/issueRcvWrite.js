/*
 * @(#)issueRcvWrite.js     2023-04-03 오전 7:57
 *
...
$(function () {
    ...
    let IssueRcvWriteModel = function () {

        const ISSUE_INFO_URL = paragonCmm.getUrl("/support/issue/viewInfo/json");
        const ISSUE_RCV_URL = paragonCmm.getUrl("/support/issue/insertRcv/json");
        ...
        return {
            ...
            saveData: function (options) {
                submitAjax(options, ISSUE_RCV_URL);
            }
        };
    };

    let IssueRcvWrite = function () {
        // ==============================================
        // attributes
        // ----------------------------------------------
        const RCV_ISSUE_FILE_TYPE = "SD/RCV";
        let model = new IssueRcvWriteModel();
        let $rootNode = $("#issueRcvWriteRootLayer");

        let $writeForm = $("#issueRcvWriteForm", $rootNode);
        let $issueTpCombo = $("select[name=rcvIssueTpCd]", $writeForm);
        let $issueDomainCombo = $("select[name=rcvIssueDomainCd]", $writeForm);
        let $reqEndDte = $("input[name=schedulEndDte]", $writeForm);
        ...
        let initFile = function () {
            htmlUtils.loadFileHtml("issueRcvFile", {
                relUid: openerData.issueId,
                fileTpCd: RCV_ISSUE_FILE_TYPE,
                defaultRelUid: ""
            });	//-- 첨부파일 로드
        };
        ...
        let initDatebox = function () {
            $reqEndDte.datebox({validType: "validDate", prompt: "estimated completion date"});
        };
        ...
        let setData = function (data) {
            ...
            $reqEndDte.datebox("setValue", getDefDate(1));
        }

        const validateData = function (data) {

            if (paragonCmm.isEmpty(data.rcvIssueTpCd)) {
            ...
            if (paragonCmm.isEmpty(data.rcvIssueDomainCd)) {
        ...
        let saveData = function () {
            ...
            if (validateData(data)) {
                $.messager.confirm("Confirm", paragonCmm.getLang("M.ALERT_SUBMIT", "L.접수"), function (r) {
                    if (r) {
                        model.saveData({
                            params: data,
                            cbFnc: function (json) {
        ...
        let setBtnEvent = function () {
            ...
            $rootNode.on("click", "button", function () {
                ...
                switch (btnId) {
                    case "issueRcvWriteCloseBtn":
                        closeForm();
                        break;
                    case "issueRcvWriteSaveBtn":
                        saveData();
                        break;
                    default:
    ...
    let issueRcvWrite = new IssueRcvWrite();
    issueRcvWrite.init();
});